...
Switch bt = Switch(6); // Button right
...
void processWait()
{
  ...
  if (bt.singleClick() || btl.singleClick())
  {
...
void processActive()
{
  byte timeValue = (activeTime - millis()) / 1000;
  if (stimevalue != timeValue)
  {
    stimevalue = timeValue;
    dbgOut(F("user tv: "));
  ...
  if ((bt.singleClick() && (activeButton == 1)) || (btl.singleClick() && (activeButton == 0)))
  {
    game.setSolved();
  }
  // Wrong button clicked
  if ((bt.singleClick() && (activeButton == 0)) || (btl.singleClick() && (activeButton == 1)))
  {
    game.setStrike();
  }
  // Time over
