...
func main() {
	...
	words, err := readLines("words6.txt")
	if err != nil {
		panic(err)
	}
	...
	filename := "../firmware/password/src/passwords6.h"
	os.Remove(filename)
	...
	encode(f, "\r\n")
	encode(f, fmt.Sprintf("const byte PWD6_COUNT = %d;\r\n", len(wordlist)))
	ids := make([]string, 0)
	for x, word := range wordlist {
		id := fmt.Sprintf("wd6_%.2d", x)
		encode(f, fmt.Sprintf("const char %s[] PROGMEM = \"%s\";\r\n", id, word.word))
		ids = append(ids, id)
		for y := range 6 {
			id := fmt.Sprintf("wd6_%.2d_%.2d", x, y)
	...
	encode(f, "\r\n")
	encode(f, "PGM_P const pwd6[] PROGMEM = {")
	for x, id := range ids {
		if x%6 == 0 {
			encode(f, "  ")
		}
			...
			for a2 := range len(alphas[2]) {
				for a3 := range len(alphas[3]) {
					for a4 := range len(alphas[4]) {
						for a5 := range len(alphas[5]) {
							my := string(alphas[0][a0]) + string(alphas[1][a1]) + string(alphas[2][a2]) + string(alphas[3][a3]) + string(alphas[4][a4]) + string(alphas[5][a5])
							_, ok := wordmap[my]
							if ok && (my != word) {
								return false
							}
